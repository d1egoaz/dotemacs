...
(defun diego/url-to-markdown-image ()
  ...
  (interactive)
  (kill-new (format "<img src=\"%s\" width=\"50%%\" />" (current-kill 0))))
...
(defun diego/what-the-commit ()
  (interactive)
  (insert
   (with-current-buffer (url-retrieve-synchronously "http://whatthecommit.com")
...
(defun diego/make-orgcapture-frame ()
  ...
  (interactive)
  (make-frame '((name . "alfredoc") (width . 80) (height . 16) (top . 400) (left . 300)))
...
(defun diego/open-url ()
  ...
  (let ((site
    ...
    (browse-url (url-encode-url (format site (read-from-minibuffer "Query: "))))))
...
(defun diego/auth-source-get-password (host user)
  ...
  (let* ((secret (plist-get (car (auth-source-search :max 1 :host host :user user)) :secret)))
    (if (functionp secret)
        (funcall secret)
      secret)))
