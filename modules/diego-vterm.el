...
(use-package vterm
  ...
  (cl-defun
   diego/vterm (&optional (buffername "*diego/vterm*") command)
...
Rename buffer if BUFFERNAME is passed, and run the passed COMMAND."
   ...
   (let ((name
          (if current-prefix-arg
              (generate-new-buffer-name buffername)
            buffername)))
     (if (get-buffer name)
...
Receives an optional BUFFERNAME to be used."
    (interactive (list (compilation-read-command nil) "*vterm*"))
  ...
  (defun diego/vterm-run-current-line ()
    ...
    (let* ((line
      ...
      (save-window-excursion (diego/vterm-run-command command "*vterm*"))))
