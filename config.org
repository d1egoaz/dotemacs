...
;; (custom-set-faces
;; `(font-lock-keyword-face ((,class :foreground "#fb4933")))
;; `(font-lock-string-face ((,class :foreground "#b8bb26")))
;; `(font-lock-comment ((,class :foreground "#F8F8F0")))
;; ))
...
(setq display-buffer-alist
      `(
        ...
        ;; every new buffer/file goes to a tab given by the root project
        ;; idea from https://emacs.stackexchange.com/a/64486
        ((lambda (b _a)
           (or (string-prefix-p "magit:" b)
               ;; (string-prefix-p "vterm" b)
               (buffer-local-value 'buffer-file-name (get-buffer b)))) ; (buffer-file-name (get-buffer b)))) ;; project-current
         (display-buffer-in-tab)
         (tab-name . diego/tab-name-for-buffer))
        ;; end display-buffer-alist
        ))
#+end_src
