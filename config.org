...
- [[#completion][Completion]]
  - [[#enhance-m-x-smexel][Enhance M-x (smex.el)]]
...
- [[#search-and-replace][Search And Replace]]
  ...
  - [[#projectile-ignored-directories][Projectile ignored directories]]
...
increased by our more expensive LSP module, and where needed.
...
(use-package modus-themes
  ...
  (setq modus-themes-headings ; Lots of options---continue reading the manual
        '((1 . section)
          (2 . section-no-bold)
          (3 . rainbow-line)
  ...
  (setq modus-themes-region 'bg-only-no-extend) ; {nil,'no-extend,'bg-only,'bg-only-no-extend}
  ...
  (setq modus-themes-syntax 'all-syntax) ; Lots of options---continue reading the manual
...
(use-package persistent-scratch
  ...
  (defun diego/make-new-scratch-buffer-go-babel ()
    ...
    (interactive)
    (switch-to-buffer (make-temp-name "*scratch-go-"))
    (insert "
\#+begin_src go
package main
import \"fmt\"
...
(use-package evil
  ...
  (setq evil-want-minibuffer nil) ; so I can map C-a, C-j, C-k
...
[[https://github.com/nonsequitur/smex/blob/master/README.markdown][Smex]] is a package the makes M-x remember our history.
Now M-x will show our last used commands first.
...
I currently use Ivy, Counsel, and Swiper to navigate around files, buffers, and projects super
quickly. Here are some workflow notes on how to best use Ivy:

While in an Ivy minibuffer, you can search within the current results by using S-Space.  To quickly
jump to an item in the minibuffer, use C-' to get Avy line jump keys.  To see actions for the
selected minibuffer item, use M-o and then press the action's key.  Super useful: Use C-c C-o to
open ivy-occur to open the search results in a separate buffer. From there you can click any item to
...
Ivy is a completion framework. Ivy comes with Counsel and Swiper.
Ivy doesn't try to do too many things, instead it provides an interface to *list, search, filter and perform actions on a collection of "things"*.
...
**** Usage
***** Creating a list
Ivy is simple to work with. The following code creates a simplified version of the buffer list using Ivy.
...
***** Actions
The ivy-read function takes a number of optional keyword arguments after the collection of results.

Actions invoke a function on the selected candidate. The default action is bound to RET. When working in the Ivy minibuffer, hit M-o to see available actions.

#+begin_example elisp :export none
(ivy-read "My buffers: "
...
(use-package ivy
  ...
  (setq ivy-wrap t) ; cycle past the last and the first candidates respectively.
  ...
  ;; require this.
  (setq ivy-extra-directories nil); Remove ./ and ../ from file commands
  (setq ivy-height 17)
  (setq ivy-fixed-height-minibuffer t) ; make sure it always uses the same height
  ;; enable ability to select prompt (alternative to `ivy-immediate-done')
  (setq ivy-use-selectable-prompt t)
  ;; The following line removes '^' in things like counsel-M-x and other ivy/counsel prompts. Most of the
  ;; time, I'm searching for a command without knowing what it begins with.
...
Counsel comes with Ivy and provides some useful commands, such as counsel-M-x and counsel-find-file.
These functions use the ivy-read function and provide useful actions on the selection candidate.
...
I switch to this buffer and press C-x C-q (ivy-wgrep-change-to-wgrep-mode) to edit it. I can now change each variable name globally using normal search and replace techniques.
...
(general-define-key
 ...
 "aP"  #'(counsel-list-processes :which-key "Kill process")
 ...
 "fj"  #'(counsel-file-jump :which-key "Jump to file curr dir")
 ...
 "hf"  #'(counsel-describe-function :which-key "Describe function/macro")
 "hF"  #'(counsel-describe-face :which-key "Describe face")
 "hk"  #'(helpful-key :which-key "Describe key")
 "hl"  #'(counsel-find-library :which-key "Describe library/package")
 ...
 "iu"  #'(counsel-unicode-char :which-key "Unicode char")
 "ir"  #'(counsel-yank-pop :which-key "Register")
 ...
 "jr"  #'(counsel-buffer-or-recentf :which-key "Jump to recent file/buffer")
 ;; LSP
 ...
 "pb"  #'(counsel-projectile-switch-to-buffer :which-key "Project buffer")
 ...
 "ps"  #'(counsel-projectile-rg :which-key "Project search")
...
;; Minibuffer
(general-define-key :keymaps '(evil-ex-completion-map evil-ex-search-keymap)
                    :states  'insert
