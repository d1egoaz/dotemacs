...
(defun diego/toggle-diego-tabs-as-workspaces ()
  "Toggle diego-tabs-as-workspaces-enabled."
  (interactive)
  (setq diego-tabs-as-workspaces-enabled (not diego-tabs-as-workspaces-enabled))
  (message "Tabs as workspaces %s" (if diego-tabs-as-workspaces-enabled "enabled" "disabled")))
...
(setq display-buffer-alist
      `(
        ...
        ((lambda (b _a)
           ...
           (and diego-tabs-as-workspaces-enabled
...
;;;; Indicators for `tab-bar-format' --- EXPERIMENTAL
  ...
  (defun diego--tab-insert-icon (name)
    `((global menu-item ,(propertize (concat " " (all-the-icons-octicon name)) 'face '(modus-themes-subtle-magenta :height 1.3 :family "github-octicons")) ignore)))
  ...
  (defun diego-tab-format-vc ()
    ...
    `((global menu-item ,(propertize (concat (format-mode-line vc-mode) " ") 'face '(modus-themes-subtle-magenta :height 0.7)) ignore)))
  ...
  (defun diego-tab-insert-file-type-icon ()
    "Format file type for the tab bar to insert file icon."
      (ignore-errors
    `((global menu-item ,(propertize (concat " " (all-the-icons-icon-for-buffer)) 'face '(modus-themes-special-mild :height 1.0 :family "github-octicons")) ignore))))

  (defun diego-tab-format-buffer-id ()
    "Buffer true name for files or just the buffer name."
    `((global menu-item ,(propertize (if buffer-file-truename
                                                 (concat " File: ["
                                     ...
                                     'face '(modus-themes-special-mild :height 0.8 :weight bold)) ignore)))
...
#+begin_src elisp
(use-package kubel
  :straight (kubel :host github :repo "d1egoaz/kubel" :branch "origin/test")
  :config
  ...
  (setq-default kube-namespace "cloudbuddies")
...
(general-define-key
 ...
 "TAB t"   #'(diego/toggle-diego-tabs-as-workspaces :which-key "Toggle tabs as workspaces")
 ;; Apps
 ...
 "t TAB"   #'(diego/toggle-diego-tabs-as-workspaces :which-key "Toggle tabs as workspaces")
 ;; Quit/Restart
...
(use-package diff-hl
  :after magit
  :hook ((prog-mode-hook       . diff-hl-prog-mode)
         (gfm-mode-hook        . diff-hl-prog-mode)
         (text-mode-hook       . diff-hl-mode)
         (git-commit-mode-hook . diff-hl-mode)
