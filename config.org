...
- [[#pending-tasks-49][Pending tasks]]
...
(defun diego--treat-chars-as-word-char ()
  ...
  (modify-syntax-entry ?- "w"))
;; TODO: @d1egoaz 2021-06-09: disabled as it's conflicting with orderless
...
*** Bind evil-jump-forward for GUI
...
(defun diego/dev-run (cmd)
  ...
  (diego/compile (concat "/opt/dev/bin/dev " cmd)))

(defun diego/dev ()
  "Runs a dev command."
  (interactive)
  (diego/compile (concat "/opt/dev/bin/dev " (completing-read "Dev command:" '(up down build test)))))
